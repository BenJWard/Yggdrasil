...
// Helper function to search our linked lists of volume mappings
struct map_list * find_map_path(struct map_list * root, const char * map_path) {
  if (root == NULL) {
    return NULL;
  }
  if (strcmp(root->map_path, map_path) == 0) {
    return root;
  }
  return find_map_path(root->prev, map_path);
}
...
static int num_subdirs(const char * path) {
  DIR * dir_obj = opendir(path);
  check(dir_obj != NULL);

  int num_dirs = 0;
  struct dirent *dir_ent;
  while (dir_ent = readdir(dir_obj)) {
    // Skip hidden things like `.` and `..`
    if (dir_ent->d_name[0] == '.') {
      continue;
    }
...
static const char * coalesce_shard_paths(const char * shard_dir_path) {
  static char cpath[5*PATH_MAX];

  DIR * dir_obj = opendir(shard_dir_path);
  check(NULL != dir_obj);

  // Reset coalesced path to zero...
  cpath[0] = '\0';
  size_t csp_len = 0;

  struct dirent * dir_ent;
  while (dir_ent = readdir(dir_obj)) {
    const char * shard_name = dir_ent->d_name;

    // Skip hidden folders (including '.' and '..')
    if (shard_name[0] == '.' || dir_ent->d_type != DT_DIR) {
      continue;
    }

    csp_len += snprintf(&cpath[csp_len], sizeof(cpath) - csp_len, ":%s/%s", shard_dir_path, shard_name);
  }

  // &[1] to skip the first comma.  :P
  return &cpath[1];
}
...
static void mount_rootfs_and_shards(const char * root_dir, const char * dest,
  ...
  while (current_entry != NULL) {
    ...
    // take the path relative to dest
    while (inside[0] == '/') {
      inside = inside + 1;
    }
    snprintf(path, sizeof(path), "%s/%s", dest, inside);
  ...
  // Each mapping is putting things into places like /opt/<target>/GCC-v8.1.0,
  // or /opt/<target>/BaseCompilerShard-v2018.08.27, etc...  We're going to
  // coalesce each /opt/<target> directory together into a single overlayfs here.
  char opt_path[PATH_MAX];

  snprintf(opt_path, sizeof(opt_path), "%s/opt", dest);
  DIR *opt = opendir(opt_path);

  // If there's no /opt directory, then don't try to coalesce them
  if (opt != NULL) {
    // Iterate over /opt
    struct dirent *opt_ent;
    while (opt_ent = readdir(opt)) {
      // Skip hidden files (including '.' and '..')
      if (opt_ent->d_name[0] == '.') {
        continue;
      }

      // target_path will be things like /opt/x86_64-linux-gnu
      char target_path[PATH_MAX];
      snprintf(target_path, sizeof(target_path), "%s/opt/%s", dest, opt_ent->d_name);

      // If there are multiple subdirectories in this directory, then coalesce them!
      int subdirs = num_subdirs(target_path);
      if (subdirs > 1) {
        // Coalesce things such that we're building overlays that go from:
        //   "/opt/<target>/foo,/opt/<target>/bar" -> "/opt/<target>"
        mount_overlay(coalesce_shard_paths(target_path), target_path, basename(target_path), work_dir, uid, gid);
      } else if (subdirs == 1) {
        // If there's only one subdirectory, then just bind-mount it back directly:
        DIR *subdir_obj = opendir(target_path);
        
        const char * subdir_name = NULL;
        while (subdir_name == NULL) {
          struct dirent *subdir_ent = readdir(subdir_obj);
          if (subdir_ent->d_name[0] == '.' || subdir_ent->d_type != DT_DIR) {
            continue;
          }
          subdir_name = subdir_ent->d_name;
        }
        char subdir_path[PATH_MAX];
        snprintf(subdir_path, PATH_MAX, "%s/%s", target_path, subdir_name);
        check(0 == mount(subdir_path, target_path, "", MS_BIND, NULL));
      }
    }
  }
}
